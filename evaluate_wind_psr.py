...
x_train, u_train, train_test_loaders = wind_dataset_instance.prepare_data_random(0.2)
...
for x_test_batch, u_test_batch in test_loader:
    ...
    plt.subplot(1, 2, 1)
    plt.scatter(u_test_batch[:5000, 0], predictions[:5000, 0], alpha=0.5)
    plt.xlabel('True Values')
    ...
    # Set font size and output directory
    font_size = 16
    ...
    # Plot the predictions vs target values
    plt.figure(figsize=(12, 6))
    plt.suptitle("MLP-DNN for Wind Power Forecast", fontsize=font_size + 4)
    # Plot first feature vs target
    plt.subplot(1, 2, 1)
    plt.scatter(u_test_batch[:5000, 0], predictions[:5000, 0], alpha=0.5, color='royalblue')
    ...
    plt.subplot(1, 2, 2)
    errors = predictions[:, 0] - u_test_batch[:, 0]
    plt.hist(errors, bins=30, edgecolor='k', color='lightcoral')
    plt.xlabel('Prediction Error', fontsize=font_size)
    ...
    plt.tight_layout()
    plt.savefig(f'{output_dir}/histogram_prediction_MLP_error.png', bbox_inches='tight')
    plt.show()
...
plt.plot(all_true_values_original_scale[400:500], label='Measured Wind Power ', alpha=1.0,
         linewidth=3.5, linestyle='-', color='green'
         )

# Plot predicted values with a dashed dark red line
plt.plot(all_predictions_original_scale[400:500], label='MLP-DNN  Predicted Wind Power ', alpha=1.0,
         linewidth=3.5, linestyle='--', color='black'
         )

# Set the title and labels with larger fonts
plt.title('MLP-DNN Model Predictions vs Measured Power (KWh)', fontsize=18)
...
plt.savefig(f'{output_dir}/MLP_vs_True.png', bbox_inches='tight')
